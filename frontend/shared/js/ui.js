...
// ── 全局偏好落库助手 ─────────────────────────────────────────────────────
// 服务端 prefs 为真源（跨端同步），localStorage 作同步镜像缓存 —— 保证首屏应用
// 主题/动画等设置无网也能秒渲染、无闪烁。改动写 localStorage 立刻生效 + 异步 PUT
// 到服务端；启动时后台拉一次覆盖本地。
const _prefLs = (k, d) => { try { return localStorage.getItem(k) || d; } catch (_) { return d; } };
function _prefWriteLocal(k, v) { try { localStorage.setItem(k, v); } catch (_) {} }
function _prefPutServer(k, v) {
  try {
    fetch("/api/prefs/" + encodeURIComponent(k), {
      method: "PUT", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ value: v }),
    });
  } catch (_) {}
}
// 拉一次服务端值覆盖本地缓存。onChange(newVal) 用于让调用方在拉到新值后同步 UI。
async function preloadPref(key, onChange) {
  try {
    const r = await fetch("/api/prefs/" + encodeURIComponent(key));
    if (!r.ok) return;
    const j = await r.json();
    const v = j && typeof j.value === "string" ? j.value : "";
    if (!v) return;              // 服务端未设 → 不动本地默认
    const before = _prefLs(key, "");
    _prefWriteLocal(key, v);
    if (before !== v && typeof onChange === "function") onChange(v);
  } catch (_) {}
}
// 保存偏好：写 localStorage（同步）+ 异步 PUT。缺网/后端断也不影响本地体验。
function savePref(key, value) {
  _prefWriteLocal(key, value);
  _prefPutServer(key, value);
}

// ── 主题：style(玻璃/拟态/报纸) × mode(光明/黑暗) × anim(动画速率) ──
// 全部走 prefs（跨设备一致），本地保留缓存供首屏无闪应用。
const STYLE_KEY = "ailogy:style", MODE_KEY = "ailogy:mode", ANIM_KEY = "ailogy:anim";
const curStyle = () => _prefLs(STYLE_KEY, "glass");
const curMode = () => _prefLs(MODE_KEY, "light");
const curAnim = () => _prefLs(ANIM_KEY, "default");   // 动画速率档：graceful / default / rapid
...
function setMode(key) {
  if (key === curMode()) return;
  savePref(MODE_KEY, key);
  applyTheme();
  showToast(`已切换为「${MODE_LABEL[key]}」模式`, { title: "主题" });
}
function setAnim(key) {
  if (key === curAnim()) return;
  savePref(ANIM_KEY, key);
  applyTheme();
  showToast(`已切换为「${ANIM_LABEL[key]}」动画速率`, { title: "动画" });
}

// 页面启动后异步拉服务端主题（跨设备同步）；服务端与本地不一致就静默切主题。
// 放到 applyTheme 后台执行，不阻塞首屏。
function _preloadThemePrefs() {
  const applyIfChanged = () => applyTheme();
  preloadPref(STYLE_KEY, applyIfChanged);
  preloadPref(MODE_KEY, applyIfChanged);
  preloadPref(ANIM_KEY, applyIfChanged);
}
...
// 早期应用主题，避免首屏闪烁（各页面 <head> 里也可内联一份），随后异步拉服务端偏好覆盖本地
applyTheme();
_preloadThemePrefs();
